    ...
    element.forEach((e, i) => {
      ...
      if (!this.#cursorOut || !this.excludeFollowElementClass) {
        // si el custor no passa per un exclude O no hi ha excludes
          let x = Math.round(e.getBoundingClientRect().left + e.clientWidth / 2);
          let y = Math.round(e.getBoundingClientRect().top + e.clientHeight / 2);
          let mx=Math.round(ev.pageX);
          let my=Math.round(ev.pageY);
